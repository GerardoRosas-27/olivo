...
/** Load bundled `migrations/*.sql` (same glob as PGLite). */
...
/**
 * Apply pending migrations to a Postgres pool (Neon / Railway).
 * Same bookkeeping as `scripts/migrate.mjs` + PGLite path.
 */
  ...
  try {
    ...
    for (const { name, path } of pendingMigrations(Object.keys(migrations), applied)) {
      try {
        ...
        console.error(`[db] neon migrate failed on ${name}`);
        throw err;
      }
      console.info(`[db] neon applied ${name}`);
      count += 1;
    }
    if (count) console.info(`[db] neon migrations done — ${count} applied`);
  } finally {
    client.release();
  }
}
...
function createNeonSql(): Promise<Sql> {
  globalRef.__pgSqlPromise__ ??= (async () => {
    ...
    // Startup migrations (not only at build): serialize like PGLite.
    const migrate = () => applyNeonMigrations(pool);
 ...
 *
 * Opens the DB and applies pending `migrations/*.sql` on **both** PGLite and
 * Neon/Postgres (`DATABASE_URL`). Idempotent — concurrent callers share one
 * promise. Vite `configureServer` awaits this at dev startup; production
 * imports kick it off immediately (see bottom of file) so signup is fast.
 */
export function ensureDbReady(): Promise<void> {
  return getSql().then(() => undefined);
}

// Server-only eager start: kick DB bootstrap + migrations as soon as this
// module loads in Node (PGLite or Neon). Client bundles never hit this path
// (`getSql` throws in the browser).
